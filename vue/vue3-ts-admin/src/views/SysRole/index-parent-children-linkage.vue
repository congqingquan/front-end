<template>
  ...
  <a-modal v-model:open="addUpdateModalOpenRef"
      ...
      <a-form name="basic" :model="addUpdateModalFormState"
          ...
          <a-form-item label="用户主键" name="roleId" hidden>
          ...
          <a-form-item label="父子联动">
              <a-switch v-model:checked="enableParentChildrenLinkageRef" @change="onEnableParentChildrenLinkageChange"/>
          </a-form-item>

          <a-form-item label="菜单资源" name="menuResources">
              <a-tree-select
                  v-model:value="addUpdateModalFormState.menuResourcesLabelInValue"
                  @change="(value, label, extra) => {
                      if (!enableParentChildrenLinkageRef) {
                          addUpdateModalFormState.menuResources = value.map((v: {value: string}) => v.value)
                      } else {
                          addUpdateModalFormState.menuResources = value
                      }
                  }"
                  show-search
                  ...
                  multiple
                  tree-checkable
                  :treeCheckStrictly="!enableParentChildrenLinkageRef"
...
const showAddUpdateModal = (type: ModalType, record?: SysRolePageVO) => {
  ...
  if (record) {
      Object.assign(addUpdateModalFormState, {
          ...record,
          menuResources: record.menuResources?.map(menu => menu.resourceId),
          menuResourcesLabelInValue: record.menuResources?.map(menu => { return {label: menu.name, value: menu.resourceId} }),
...
const enableParentChildrenLinkageRef = ref<boolean>(false)
const onEnableParentChildrenLinkageChange = () => {
  // Object.assign(addUpdateModalFormState, { ...addUpdateModalFormState })
}
...
interface SysRoleModalFormState {
  ...
  // 回显时使用，为了配合 TreeSelect 组件带有 treeCheckStrictly 属性的场景（父子不联动）
  menuResourcesLabelInValue: { label: string, value: string | number }[]
  apiResources: string[]
}
      ...
      switch (modalTypeRef.value) {
          case 'ADD': 
              await Api.addSysRole(
                  {
                      ...addUpdateModalFormState,
                      menuResources: addUpdateModalFormState.menuResources?.map(menuResourceId => parseInt(menuResourceId)),
                      apiResources: addUpdateModalFormState.apiResources?.map(apiResourceId => parseInt(apiResourceId))
                  }
              )
              break
